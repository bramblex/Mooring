...
- Harbor 可以在启动或主窗口关闭后的第一次扫描中拆掉旧 Chrome Group 投影。
- 拆掉旧投影只解除 Chrome Group，不删除 bookmark，不关闭 Chrome Tab。
...
- 已打开的 Chrome Tabs 的处理策略需要明确：第一版可以关闭它们，也可以先移动到 Unmanaged；实现前必须在任务中指定。
...
Harbor 同时维护 Workspace 顺序、Pinned Page 顺序、Temp Page 顺序和 Chrome 运行时顺序。

### Workspace 排序

- Workspace 长期顺序来自 `Harbor Workspace` 根目录下的书签文件夹顺序。
- 拖动 Workspace 时，同步移动书签文件夹。
- 如果 Workspace 当前有 Chrome Group，运行时也尽量移动 Chrome Group。
- Chrome Group 排序不是长期来源。

### Workspace 与 Unmanaged 的位置

- Harbor 管理的 Workspace 区域排在 Unmanaged 区域前面。
- Workspace 管理的 Chrome Group 应该位于主窗口 Chrome Tabs / Chrome Groups 的前部区域。
- 如果 managed Chrome Group 被拖到后面，下一次同步时移回 Workspace 区域末尾。
- 如果 unmanaged Chrome Tab 或 Chrome Group 被拖进 managed 区域，下一次同步时推回 Workspace 区域之后，除非用户明确把它加入 Workspace。
...
- 如果 Pinned Page 当前绑定 Chrome Tab，移动 Pinned Page 时同步移动 Chrome Tab。
...
- 如果该 Pinned Page 当前绑定 Chrome Tab，同时把 Chrome Tab 移入目标 Workspace 对应 Chrome Group。
...
- Temp Page 顺序来自 Chrome Tab 顺序。
- Temp Page 不能拖到 Pinned 区。
- Temp Page 如需进入 Pinned 区，必须通过星标固定。
- Temp Page 移动只调整 Chrome Tab 顺序，不写 bookmark。

### 恢复位置

点击关闭态 Pinned Page 时，新 Chrome Tab 应尽量回到 bookmark 对应位置：

1. 找该 Pinned Page 前面最近一个已打开 Pinned Page。
2. 如果找到，插到它对应 Chrome Tab 后面。
3. 如果前面没有，找后面最近一个已打开 Pinned Page。
4. 如果找到，插到它对应 Chrome Tab 前面。
5. 如果 Workspace 里没有其他已打开 Pinned Page，放到 Chrome Group 最前面。
...
Runtime 层可以随主窗口关闭、Chrome 重启、service worker 重启而重建。
