...
- 左下角 AI / command 类入口使用 dock 形态：主按钮固定左下角，输入气泡从按钮右侧展开，底部与按钮对齐；输入内容增多时向上增长。
- AI / command dock 的设置入口放在主按钮正上方，设置面板从同一左侧锚点展开；Esc 必须同时收起输入气泡和设置面板。
- AI / command 输入框使用现有 control 风格：6px 圆角、普通边框、`--color-control-bg` 背景、现有 focus/hover 色，不使用独立品牌色、渐变、玻璃拟态或重阴影。
- Dock / popover / preview 的视觉必须服从侧边栏现有密度：轻薄边框、小间距、浅阴影或无阴影，不做居中 modal，除非操作本身需要阻塞全局。
- 多行 prompt 输入按 `Enter` 执行、`Shift+Enter` 换行、`Esc` 收起；执行前后预览和确认按钮在输入框上方展示，确认后才调用模型动作执行链路。
